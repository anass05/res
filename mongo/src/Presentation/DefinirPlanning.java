...
public class DefinirPlanning extends JFrame {
    ...
    public DefinirPlanning(Module module, Controleur controleur) {
        super("Définir un planning au module " + module.getLibele());
        this.module = module;
...
//        table.setRowSelectionInterval(0, 0);
        ...
        lblCours.setFont(new Font("Tahoma", Font.PLAIN, 13));
        lblCours.setHorizontalAlignment(SwingConstants.TRAILING);
        panel_1.add(lblCours);

        JPanel panel_01 = new JPanel();
        panel_01.setBorder(new EmptyBorder(0, 2, 0, 20));
        panel_1.add(panel_01);
        panel_01.setLayout(new BoxLayout(panel_01, BoxLayout.X_AXIS));

        coursProgress = new JProgressBar();
        coursProgress.setForeground(new Color(255, 153, 102));
        ...
        panel_11.setBorder(new EmptyBorder(0, 2, 0, 20));
        panel_1.add(panel_11);
        panel_11.setLayout(new BoxLayout(panel_11, BoxLayout.X_AXIS));

        tdProgress = new JProgressBar();
        tdProgress.setForeground(new Color(255, 153, 102));
        tdProgress.setValue((int) (TDLeft * 100 / TDCurrent));
        panel_11.add(tdProgress);


        JLabel lblTp = new JLabel("TP:  ");
        lblTp.setFont(new Font("Tahoma", Font.PLAIN, 13));
        lblTp.setHorizontalAlignment(SwingConstants.TRAILING);
        panel_1.add(lblTp);

        panel = new JPanel();
        panel.setBorder(new EmptyBorder(0, 2, 0, 20));
        panel_1.add(panel);
        panel.setLayout(new BoxLayout(panel, BoxLayout.X_AXIS));

        tpProgress = new JProgressBar();
        tpProgress.setForeground(new Color(255, 153, 102));
        tpProgress.setValue((int) (TPLeft * 100 / TPCurrent));
        panel.add(tpProgress);


        JLabel lblProjet = new JLabel("Projet:  ");
        lblProjet.setFont(new Font("Tahoma", Font.PLAIN, 13));
        ...
        projetProgress = new JProgressBar();
        projetProgress.setForeground(new Color(255, 153, 102));
        projetProgress.setValue((int) (PLeft * 100 / PCurrent));
        panel_2.add(projetProgress);


        JPanel rightPannel = new JPanel();
        rightPannel.setBorder(new EmptyBorder(10, 10, 10, 10));
        rightPannel.setLayout(new BoxLayout(rightPannel, BoxLayout.Y_AXIS));
        rightPannel.add(panel_1);
        JPanel pan2 = new JPanel();
        pan2.setBorder(new EmptyBorder(10, 0, 0, 0));
        JButton valider = new JButton("Valider");
        JButton annuler = new JButton("Annuler");
        JButton editerCH = new JButton("Editer Charge horaire");
        ...
        editerCH.addActionListener(e -> controleur.edtierChargeHoraire(module));
        ...
        for (int count = 0; count < model.getRowCount(); count++) {
            ...
            for (String d : ss) {
                if (!d.equals("") && !d.equals("TP") && !d.equals("TD") && !d.equals("Projet")) {
                    module.getCahierCharges().getChapitre(d).setSeance(p.getSeanceAt(count + 1));
                } else if (d.equals("TP") || d.equals("TD") || d.equals("Projet")) {
                    p.getSeanceAt(count + 1).addChapitre(new Chapitre(d));
                }
            }
        }
        controleur.insertPlanning(module);
    }
}
